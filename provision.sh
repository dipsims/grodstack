...
until docker exec -i edx.live.mysql mysql -uroot -se "SELECT EXISTS(SELECT 1 FROM mysql.user WHERE user = 'root')" &> /dev/null
do
  printf "."
  sleep 1
done
...
docker exec -i edx.live.mysql mysql -uroot mysql < provision.sql

# If necessary, ensure the MongoDB server is online and usable
# and create its users.
if needs_mongo "$to_provision"; then
	echo -e "${GREEN}Waiting for MongoDB...${NC}"
	until docker exec -i edx.live.mongo mongo --eval "printjson(db.serverStatus())" &> /dev/null
	do
	  printf "."
	  sleep 1
	done
	...
	docker exec -i edx.live.mongo mongo < mongo-provision.js
else
