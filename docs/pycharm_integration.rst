...
- Environment variables:
  ...
  - ``PROD_VOLUME=/LOCAL/PARENT/PATH/TO/workspace`` (i.e.: Path to where your local repositories are cloned)
...
**Some Tips**: If your remote isn't loading you may need to set your PROD_VOLUME variable globally in your ./bash_profile. Additionally try reseting docker as a last resort and things should sync successfully after that.
...
that you have defined the PROD_VOLUME environment variable correctly (i.e. to the root of your workspace where all repositories are checked out).
...
3. Make sure that the Container path and Host path are right. Normally you should have a line mapping /edx/app/edxapp/edx-platform to the related local source folder (i.e. often PROD_VOLUME/edx-platform).
